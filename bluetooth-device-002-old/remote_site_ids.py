#!/usr/bin/env python3
"""
Remote Site Bluetooth Intrusion Detection
...
class RemoteSiteIDS:
    def __init__(self):
        # Configuration
        self.trigger_threshold = 45      # 45 seconds constant detection
        self.scan_duration = 5           # 5 second scans
        self.scan_interval = 3           # 3 seconds between scans
        self.relay_pin = 18
        self.relay_active_high = True
        self.alarm_duration = 300        # 5 minutes
        
        # Get Pi's own Bluetooth MAC to ignore
    ...
    def get_pi_mac(self):
        """Get Pi's own Bluetooth MAC address to ignore"""
        try:
            import subprocess
    ...
    def setup_gpio(self):
        ...
        try:
            ...
            self.logger.info(f"GPIO setup complete - Pin {self.relay_pin} ready for relay control")
        except Exception as e:
    ...
    def stop_alarm(self):
        ...
        if self.alarm_active:
            try:
                ...
                self.log_event("ALARM_STOPPED", "Auto-off timer or manual stop")
            except Exception as e:
                self.logger.error(f"Error stopping alarm: {e}")
    
    def manual_stop_alarm(self):
        """Manually stop alarm and reset detection"""
        self.stop_alarm()
        self.first_detection_time = None
        self.detected_devices = {}
        self.logger.info("Manual alarm stop - detection reset")
    ...
    def process_detections(self, found_devices):
        """Process detections and check for alarm trigger"""
        ...
        if found_devices:
            ...
            if self.first_detection_time is None:
                ...
                self.logger.info(f"? FIRST DETECTION: {len(found_devices)} device(s) found - monitoring...")
    ...
    async def monitoring_loop(self):
        ...
        while self.running:
            try:
                ...
                # Status update
                if found_devices:
                    status = f"? {len(found_devices)} device(s) detected"
                    if self.first_detection_time:
                        elapsed = (datetime.now() - self.first_detection_time).total_seconds()
                        status += f" (for {elapsed:.1f}s)"
    ...
    def print_status(self):
        ...
        print(f"\n{'='*50}")
        print(f"??  REMOTE SITE IDS STATUS")
        print(f"{'='*50}")
        print(f"Detection State: {'? MONITORING' if self.first_detection_time else '? CLEAR'}")
        print(f"Alarm Status:    {'? ACTIVE' if self.alarm_active else '? NORMAL'}")
        print(f"Devices Found:   {len(self.detected_devices)}")
        
        if self.first_detection_time:
            elapsed = (datetime.now() - self.first_detection_time).total_seconds()
            print(f"Detection Time:  {elapsed:.1f}s / {self.trigger_threshold}s")
        
        if self.detected_devices:
            print(f"\n? CURRENT DEVICES:")
            for mac, info in self.detected_devices.items():
                print(f"   {info['name'][:20]:<20} | {mac} | {info['signal']}dBm")
