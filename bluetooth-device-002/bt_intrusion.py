...
class ModernBluetoothIDS:
    def __init__(self, config_file="bt_ids_config.json"):
        ...
        self.baseline_devices: Set[str] = set()
        self.device_patterns: Dict[str, int] = defaultdict(int)
    ...
    def get_device_pattern(self, name: str) -> str:
        """Extract device pattern from name for smart matching"""
        ...
        patterns = [
            ...
            "galaxy", "samsung", "pixel", "android",
            "windows", "laptop", "desktop", "speaker",
            "headphones", "earbuds", "watch", "fitbit"
        ]
        ...
        # Use first word if no pattern matches
        return name_lower.split()[0] if ' ' in name_lower else name_lower
    
    def is_device_suspicious(self, device: DeviceInfo) -> bool:
        """Determine if device should trigger alarm"""
        # Skip if in learning mode
        if self.learning_mode:
            return False
        
        # Check if device pattern is known
        pattern = self.get_device_pattern(device.name)
        
        # If MAC is in baseline, not suspicious
        if device.mac in self.baseline_devices:
            return False
        
        # If pattern is common and recently seen, probably not suspicious
        if pattern in self.device_patterns and self.device_patterns[pattern] > 2:
            # Add to baseline if seen multiple times
            self.baseline_devices.add(device.mac)
            return False
        
        # Check signal strength (very weak signals might be far away)
        if device.signal_strength < self.signal_threshold:
            return False
        
        # Need multiple detections for confidence
        if device.detection_count < self.confidence_threshold:
            return False
        
        return True
    
    def trigger_alarm(self, device: DeviceInfo):
        ...
        if GPIO_AVAILABLE:
            try:
                ...
                self.logger.warning(f"ALARM TRIGGERED by {device.name} ({device.mac})")
            ...
            self.logger.warning(f"ALARM WOULD TRIGGER: {device.name} ({device.mac})")
    
    def stop_alarm(self):
        """Stop the alarm"""
        if GPIO_AVAILABLE and self.alarm_active:
            try:
                GPIO.output(self.relay_pin, not self.relay_active_high)
                self.alarm_active = False
