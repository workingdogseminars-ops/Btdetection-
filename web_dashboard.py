...
@auth.login_required
def arm_system():
    """ARM the system with manual override support"""
    try:
        next_transition = ids_controller.set_manual_override(True)
        
        if next_transition:
            message = f"System ARMED (override until {next_transition.strftime('%H:%M %A')})"
        else:
...
@auth.login_required
def disarm_system():
    """DISARM the system with manual override support"""
    try:
        next_transition = ids_controller.set_manual_override(False)
        
        if next_transition:
            message = f"System DISARMED (override until {next_transition.strftime('%H:%M %A')})"
        else:
